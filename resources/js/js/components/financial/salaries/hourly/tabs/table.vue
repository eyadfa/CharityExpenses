    ...
    <div class="card card-xl-stretch mb-5 ">
        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="me-4 btn-group">
                <button :disabled="denies('fin_salaries_hourly_search')" class="btn btn-sm btn-flex btn-light btn-active-primary fw-bolder dropdown-toggle"  type="button" data-bs-toggle="dropdown" aria-expanded="false" >
        ...
        <div class="card-body py-3">
            ...
            <div class="overflow-auto">
                <el-table
                    ...
                    <el-table-column align="right" label="عدد الساعات"   prop="hour_no" />
                    <el-table-column align="right" label=" سعر الساعة"   prop="hour_payments" />
                            ...
                            <el-dropdown v-if="scope.row.is_financial_manager_certified == 0 " trigger="click">
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu >
                                        ...
                                        <el-dropdown-item v-if="scope.row.is_certified == 0 && can('fin_salaries_hourly_edit')">
                                            <router-link :to="{name: 'editFinancialHourlySalaries', params: { id: scope.row.id }}" class="dropdown-item">
                                                تعديل
                                            </router-link>
                                        </el-dropdown-item >

                                        <el-dropdown-item v-if="scope.row.is_certified == 0 && can('fin_salaries_hourly_delete')" >
                                            <a  class="dropdown-item" @click.prevent="_delete(scope.row.id)" >
                                                حذف
                                            </a>
                                        </el-dropdown-item>

                                        <el-dropdown-item  v-if="scope.row.is_certified == 0 && can('fin_salaries_hourly_certify')" >
                                        ...
                                        <el-dropdown-item >
                                            ...
                                            && can('fin_salaries_hourly_manager_certify')" class="dropdown-item" @click.prevent="manager_certify(scope.row.id)" >
...
export default {
    ...
    data() {
        return {
            ...
            add_url: this.$router.resolve({name:'addFinancialHourlySalaries'}),
            loading:false,
    ...
    methods: {
        ...
        manager_certify(sanad_id) {
            ...
            ElMessageBox.prompt('في حالة اعتماد السند لا يمكن التعديل عليه', 'اعتماد المدير المالي', {
                ...
                .then(async ({value}) => {
                    let res = await shared.post(this.api_url + "/mcertify/" +  sanad_id, {sanad_id: sanad_id, notes: value});
                    if(res.status){
                        ElMessage({
                            type: 'success',
                            message: `تمت عملية الاعتماد بنجاح`,
